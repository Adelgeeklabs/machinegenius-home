...
import { ITwitterSharingAccount } from "@/app/(authPaths)/social-media/post-comments/PostCommentsPage";
...
interface IProps {
  ...
  dataToEdit?: ITwitterSharingAccount; // Data to edit
...
export default function SocialMediaModal({
  ...
  async function handleTwitterAccount() {
    if (
      ...
      (!addAccount1State.userName?.trim() ||
        !addAccount1State.accountName?.trim() ||
  ...
  async function handleAddAccount() {
    if (
      forWhat === "add_account" &&
      (!addAccountState.accountId?.trim() ||
        !addAccountState.accountName?.trim() ||
        !addAccountState.accountLink?.trim() ||
        !addAccountState.platform?.trim())
    ) {
      toast.error("Please fill in all fields!");
      return;
    }
