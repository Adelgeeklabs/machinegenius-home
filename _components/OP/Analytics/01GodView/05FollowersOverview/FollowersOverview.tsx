...
import { IGetYoutubeFollowersOverview } from "../../00Types/OP_Analytics_Types";
...
export default function FollowersOverview({
  fetchedYoutubeFollowersOverview,
}: {
  fetchedYoutubeFollowersOverview: IGetYoutubeFollowersOverview;
}) {
  return (
          ...
          <div className={`${styles.tableContainer} flex`}>
            <div className={styles.table + " w-full"}>
              ...
              <ul className={styles.table_header}>
                ...
                <li className={`w-[45%] ${styles.center}`}>
                  <span>Followers</span>
                </li>
              </ul>
              ...
              <div className={styles.table_body}>
                {Array.isArray(fetchedYoutubeFollowersOverview) &&
                fetchedYoutubeFollowersOverview.length > 0
                  ? fetchedYoutubeFollowersOverview.map((item, index) => {
                      return (
                        <ul key={index}>
                          <li className="w-[55%]">
                  ...
                  : [...Array(6)].map((_, i) => (
                      <li key={i} className="flex mb-[--sy-10px]">
                        <div className="w-[55%] h-4 bg-gray-300 rounded animate-pulse mr-2"></div>
                        <div className="w-[45%] h-4 bg-gray-300 rounded animate-pulse mr-2"></div>
                      </li>
                    ))}
              </div>
            </div>
