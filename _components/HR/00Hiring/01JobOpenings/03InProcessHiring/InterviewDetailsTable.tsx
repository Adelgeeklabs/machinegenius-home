...
export default function InterviewDetailsTable({
  ...
  async function acceptCandidate(candidateId: string) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/hr/candidate/next-hiring-step/${candidateId}`,
        {
          method: "PUT",
          headers: {
  ...
  async function rejectCandidate(candidateId: string) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/hr/candidate/reject/${candidateId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
    ...
    <div className={`${styles.tableContainer} h-[68vh] w-full`}>
      ...
      <div className={styles.table + " max-w-full"} id="table">
        ...
        <ul className={styles.table_header + " space-x-2"}>
          ...
          {path === "/hr/hiring/job-openings/interview-details" && (
            <li className="w-[15%]">
              <span>Date</span>
            </li>
          )}
          <li
            className={`${path === "/hr/hiring/job-openings/interview-details" ? "w-[15%]" : "w-[30%]"}`}
          >
            <span>Status</span>
          </li>
        </ul>
        ...
        <div className={styles.table_body}>
          {sortedData?.map((e: any, idx: number) => (
            <ul
              key={idx}
              className={`space-x-2 ${
                e.stepsStatus[9].status === "Accepted"
                  ? "!bg-green-500"
                  : e.stepsStatus[9].status === "Rejected"
                    ? "!bg-red-500"
                    : ""
              }`}
            >
              ...
              <li
                className={`${path === "/hr/hiring/job-openings/interview-details" ? "w-[15%]" : "w-[30%]"} flex items-center gap-2`}
              >
                <CustomBtn
                  word={statusLoading ? "Wait..." : "Accept"}
                  btnColor="black"
                  onClick={() => acceptCandidate(e._id)}
                  disabled={statusLoading}
                />
                <CustomBtn
                  word={statusLoading ? "Wait..." : "Reject"}
                  btnColor="white"
                  onClick={() => rejectCandidate(e._id)}
                  disabled={statusLoading}
                />
              </li>
            </ul>
