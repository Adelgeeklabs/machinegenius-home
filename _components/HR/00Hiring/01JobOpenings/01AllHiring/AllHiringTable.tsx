...
import { useJobCurrentStepContext } from "@/app/_context/jobCurrentStep";
...
enum HiringStepsEnum {
  ...
  Face_To_Face_Status = "Face_To_Face_Status",
  Job_Offer = "Job_Offer",
  Candidate_Offer_Status = "Candidate_Offer_Status",
  Required_Documents = "Required_Documents",
}

export default function AllHiringTable() {
  const { currentStep, setCurrentStep } = useJobCurrentStepContext();
  ...
  async function delJob(jobId: string) {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/hr/hiring/delete-job/${jobId}`,
        {
          method: "Delete",
          headers: {
  ...
  async function updateNextStep(hiringStatus: string) {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/hr/hiring/next-step/${ele._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchData = async () => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/hr/hiring/hiring?limit=1000&skip=${skip}&type=All`,
        {
          method: "GET",
          headers: {
            Authorization: "Bearer " + localStorage.getItem("token"),
  ...
  useEffect(() => {
    const currentStepMapping: Record<HiringStepsEnum, string> = {
      [HiringStepsEnum.REQUEST_HIRING]: "",
      [HiringStepsEnum.Job_Listings]: `/start-hiring/${ele?._id}`,
      [HiringStepsEnum.Get_Job_Candidates]: "/prospects",
      [HiringStepsEnum.Schedule_Interview_Call]: "/short-list",
      [HiringStepsEnum.Interview_Call_Question]:
        "/phone-interview-questionnaire",
      [HiringStepsEnum.Tasks]: "/send-task",
      [HiringStepsEnum.Schedule_Face_To_Face_Interview]:
        "/face-to-face-interview-msg",
      [HiringStepsEnum.Face_To_Face_Status]: "/interview-details",
      [HiringStepsEnum.Job_Offer]: "/offer-msg",
      [HiringStepsEnum.Candidate_Offer_Status]: "/candidate-offer-status",
      [HiringStepsEnum.Required_Documents]: "/required-documents",
    };
    ...
    } else if (ele?.currentStep) {
      router.push(
        `/hr/hiring/job-openings${currentStepMapping[ele?.currentStep]}`
      );
    }
  }, [step, ele]);

  useEffect(() => {
    console.log(step, ele);
  }, [step, ele]);
  ...
  return (
    <>
      <div className={styles.database_table}>
        ...
        <div className={styles.table_body}>
          {data.length > 0 &&
            data?.map((ele, idx) => (
              <ul
                className={`${styles.table_row} w-[100%] group`}
                key={ele._id}
              >
                <li className="w-[15%] text-center">{ele.title}</li>
                <li className="w-[15%]">{ele.level}</li>
                <li className="w-[15%]">{ele.department}</li>
                <li className="w-[15%]">
                  <span
                    style={{
                      background: ele.createdBy.theme,
                      color: ele.createdBy.theme === "#000000" ? "#fff" : "",
                    }}
                  >
                ...
                >
                  <CustomBtn
                    ...
                    onClick={() => {
                      if (
                        ele.hiringStatus === "Pending" ||
                        ele.hiringStatus === "Complete"
                      ) {
                        return;
                      }
                      setStep(ele?._id);
                      setEle(ele);
                      setCurrentStep(ele?.currentStep);
                    }}
                    className="!w-[140px] text-nowrap text-center"
                  />
                </li>
                <li className="w-[10%]">
                  <svg
      ...
      {showDeleteModal && (
        <div className="absolute bg-white bg-opacity-50 backdrop-blur-sm inset-0 z-[10]">
          <div className="px-[--20px] py-[--20px] bg-white rounded-lg border border-[#2a2b2a] w-fit absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
            <h6 className="text-[--20px] font-bold text-center mb-[--20px]">
              You will not be able to move on with the candidates of this job,{" "}
              <br />
