...
export default function ProspectsPreviewTable({
  data,
  setSelectedCandidateCV,
  setSelectedCandidateId,
}: {
  data: any;
  setSelectedCandidateCV: (cv: string) => void;
  ...
  useEffect(() => {
    if (sortedData.length > 0) {
      setSelectedCandidate((sortedData[0] as any)._id);
      setSelectedCandidateCV((sortedData[0] as any).cvLink);
      setSelectedCandidateId((sortedData[0] as any)._id);
    }
  }, [sortedData]);

  const handleCandidateSelect = (candidate: any) => {
    setSelectedCandidate(candidate._id);
    setSelectedCandidateCV(candidate.cvLink);
      ...
      <div className={styles.table + " max-w-full"} id="table">
        ...
        <div className={styles.table_body}>
          {sortedData?.map((e: any, idx: number) => (
            <ul
              key={idx}
              className={`space-x-2 ${e.stepsStatus[2].status == "Rejected" && "opacity-50"}`}
            >
              <li className="w-[5%]">
                <input
                  type="checkbox"
                  checked={selectedCandidate === e._id}
                  onChange={() => handleCandidateSelect(e)}
