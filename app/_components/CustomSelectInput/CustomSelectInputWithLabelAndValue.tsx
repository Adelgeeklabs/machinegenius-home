...
export default function CustomSelectInputWithLabelAndValue<T>(
  ...
  // Track whether selection was user-driven
  ...
  const [selectedOption, setSelectedOption] = useState<Option<T> | null>(() => {
    ...
    // Find option matching the label prop
    const matchingLabel = props.options.find(
      (option) => option.label === props.label
    );
    return matchingLabel || null;
  });

  // Handle prop label changes, especially for reset scenarios
  useEffect(() => {
    const isPlaceholder = props.label.includes("Select ");

    // If user hasn't made a selection yet OR we're resetting the field
    if (!userSelected || isPlaceholder) {
      if (isPlaceholder) {
        // Reset to null when receiving a placeholder label
        setSelectedOption(null);
        setUserSelected(false);
      } else {
        // Find a match for the non-placeholder label
        const matchingLabel = props.options.find(
          (option) => option.label === props.label
        );
        ...
        if (
          matchingLabel &&
          (!selectedOption || matchingLabel.label !== selectedOption.label)
        ) {
          setSelectedOption(matchingLabel);
        }
      }
    }
  }, [props.label, props.options, selectedOption, userSelected]);
  ...
  const handleSelectedItem = (option: Option<T>) => {
    ...
    setUserSelected(true); // Mark that user made a selection
    setIsActive(false);
    if (props.getValue) {
  ...
  const displayText = selectedOption ? selectedOption.label : props.label;
  ...
  return (
    ...
    >
      ...
      >
        {props.icon}
        <span>{truncateText(displayText, 20)}</span>
        <svg
