...
interface IProps {
  ...
  platformCards?: IPlatformCard[] | undefined;
}
...
export default function PlatformBox({
  ...
  const { brandMap } = useContext(globalContext);
      ...
      <div className="flex flex-col mt-[--sy-27px] gap-[--sy-27px] overflow-y-auto p-[--5px] pr-[--8px]">
        ...
        platformCards.length > 0 ? (
          platformCards?.map((card, i) => (
            <div
              key={i}
              className={`${styles.platformBox} flex flex-col gap-[0.8vw] !pt-[--sy-18px] cursor-pointer`}
              onClick={() => {
                if (card.subscribers !== undefined && card.subscribers > 0) {
                  router.push(
                    `/social-media/post-sharing/${platformName?.toLowerCase()}/${
                      brandMap[card.title]
                    }`,
                  );
                } else {
                  toast.error("No data found for this brand!");
                }
              }}
            >
