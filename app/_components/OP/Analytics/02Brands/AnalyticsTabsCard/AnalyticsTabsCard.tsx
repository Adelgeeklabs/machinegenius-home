...
const AreaChart1 = dynamic(
  () => import("@/app/_components/OP/Analytics/00Charts/AreaChart1"),
  {
    ssr: false,
  }
);
...
function AnalyticsTabsCard({ title, value, change, chartData, datesCategories, pageState }: IProps) {
  return (
    <div
    ...
    >
      <div className="flex justify-center items-center gap-[1.5vw] h-full">
        ...
        <div className="h-full flex justify-center items-center w-[60%]">
          <AreaChart1 seriesName={title} chartData={chartData} datesCategories={datesCategories || []} timeframe={pageState.activeAnalyticsTimeframe} />
        </div>
      </div>
    </div>
  );
...
export default memo(AnalyticsTabsCard);
