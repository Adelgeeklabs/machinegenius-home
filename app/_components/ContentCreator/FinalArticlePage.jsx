...
export default function FinalArticlePage() {
  ...
  function handleDisplayContentDataToEdit() {
    ...
    setSelectedBrandId(editContentData.brand_id._id);
  ...
  async function handleExpandContent() {
    const endpoint =
      brandIdMap[selectedBrandId] === "Little Birdie"
        ? `${process.env.NEXT_PUBLIC_API_BASE_URL}/content-creation/expand-littlebirdie-script`
  ...
  async function handleUpdateContent() {
    ...
    let postBody = {
      content_title: editContentData.content_title,
    ...
    while (attempts < maxRetries) {
      try {
        ...
        if (!res.ok && res.status !== 401) {
          ...
          if (brandIdMap?.[selectedBrandId] === "Movie Myth") {
            router.push("/content-creator/create/movie-myth/article-ready");
          } else {
            router.push("/content-creator/create/schedule-script");
          }
        }
        json = await res.json();
        if (json) {
  ...
  if (pageState.isPageLoading) {
    return (
      <div className="flex flex-col justify-center items-center h-[75vh]">
        <LogoAndTitle needTxt={false} title="Loading ..." />
      </div>
    );
  }
      ...
      <div className="flex flex-col justify-center items-center mx-auto h-[75vh] py-[1.5vw]">
        <div className={"genuisWorking"}>
          ...
          {(checkStatus.grammar === "fail" ||
            ...
            checkStatus.plagiarism !== "waiting" &&
            checkStatus.ai !== "waiting" &&
            (brandIdMap?.[selectedBrandId] === "Movie Myth" ? (
              <CustomBtn
                word={"Results"}
                btnColor="black"
