...
interface WithSortingProps {
  sortingState: Record<string, SortingState>;
  setSortingState: (field: string) => void;
}

interface WithSortingInjectedProps {
  SortingControl: React.ComponentType<WithSortingProps>;
}

export const withSorting = <P extends WithSortingInjectedProps>(
  WrappedComponent: ComponentType<P>
) => {
  return (props: Omit<P, keyof WithSortingInjectedProps>) => {
    const SortingControl = ({
      ...
      return (
        ...
        <div
          className="border-[var(--dark)] border-[1px] rounded-md flex justify-between items-center px-[0.677vw] w-[11.927vw] py-[0.37vw] cursor-pointer"
          onClick={() => setSortingState(Object.keys(sortingState)[0])}
        >
          <span>{sortingState[Object.keys(sortingState)[0]]?.direction}</span>
          <svg
    ...
    return (
      <WrappedComponent {...(props as P)} SortingControl={SortingControl} />
    );
  };
};
